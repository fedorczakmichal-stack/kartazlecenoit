...
const additivesData = {
‘KCl 7.45%’: { unit: ‘ml’, commonDoses: [‘10’, ‘15’, ‘20’, ‘25’] },
‘KCl 15%’: { unit: ‘ml’, commonDoses: [‘5’, ‘10’, ‘15’, ‘20’] },
‘MgSO4 20%’: { unit: ‘ml’, commonDoses: [‘5’, ‘10’, ‘15’, ‘20’] },
‘MgSO4 10%’: { unit: ‘ml’, commonDoses: [‘10’, ‘20’, ‘30’] },
‘CaCl2 10%’: { unit: ‘ml’, commonDoses: [‘5’, ‘10’, ‘15’] },
‘NaHCO3 8.4%’: { unit: ‘ml’, commonDoses: [‘10’, ‘20’, ‘30’, ‘50’] },
‘Heparyna’: { unit: ‘j’, commonDoses: [‘2500’, ‘5000’, ‘7500’] },
‘Insulina’: { unit: ‘j’, commonDoses: [‘10’, ‘20’, ‘30’] },
‘Soluvit N’: { unit: ‘amp’, commonDoses: [‘1’, ‘2’] },
‘Vitalipid’: { unit: ‘ml’, commonDoses: [‘10’, ‘20’] },
‘Cernevit’: { unit: ‘amp’, commonDoses: [‘1’] },
‘Addamel’: { unit: ‘ml’, commonDoses: [‘10’, ‘20’] },
‘Tracutil’: { unit: ‘ml’, commonDoses: [‘10’, ‘20’] },
‘Aminoplasmal’: { unit: ‘ml’, commonDoses: [‘250’, ‘500’] },
‘Glukoza 40%’: { unit: ‘ml’, commonDoses: [‘20’, ‘50’, ‘100’] },
‘Mannitol 20%’: { unit: ‘ml’, commonDoses: [‘50’, ‘100’] },
‘Albuminy 20%’: { unit: ‘ml’, commonDoses: [‘50’, ‘100’] },
‘Noradrenalina’: { unit: ‘amp’, commonDoses: [‘1’, ‘2’] },
‘Dopamina’: { unit: ‘amp’, commonDoses: [‘1’, ‘2’] },
‘Furosemid’: { unit: ‘amp’, commonDoses: [‘1’, ‘2’, ‘4’] }
};
...
{ time: “ciągłe”, name: “Monitorowanie hemodynamicczne” },
...
1. **Unikanie duplikatów:** Przeanalizuj listę leków już podawanych pacjentowi i zaproponuj tylko te, których brakuje. Nie powtarzaj leków z listy “Aktualne leki”.
1. **Format odpowiedzi:** Zwróć odpowiedź WYŁĄCZNIE jako obiekt JSON w następującym formacie:
   {
   “continuousDrugs”: [
...
1. **Weryfikacja:** Nazwy leków muszą być zgodne z lekami dostępnymi w Polsce (np. “Noradrenalina”, “Meropenem”, “Enoksaparyna”).
1. **Format dawkowania:** Zawsze podawaj konkretne wartości liczbowe lub zakresy (np. ‘0.1-0.5 μg/kg/min’, ‘100 mg/h’). Kategorycznie unikaj opisowych zaleceń typu ‘do uzyskania efektu’, ‘miareczkować do MAP > 65’, ‘wg kontroli glikemii’.`;
...
template.fluids?.forEach(fluid => {
    ...
    const fluidNameInput = newRow.querySelector('.fluid-name');
    const volumeInput = newRow.querySelector('input[id$="_vol"]');
    const rateInput = newRow.querySelector('input[id$="_rate"]');
    
    if (fluidNameInput) fluidNameInput.value = fluid.name;
    if (volumeInput) volumeInput.value = fluid.volume || '';
    if (rateInput) rateInput.value = fluid.rate || '';
    
    // Ustaw dodatki dla szablonów (kompatybilność wsteczna)
    if (fluid.additives) {
        setAdditivesFromText(newRow, fluid.additives);
    }
});
...
button.innerHTML = `<i class="fas fa-user-md"></i> <span>${template.name.substring(0, 30)}</span> <button class="delete-template-btn" data-template-key="${key}" title="Usuń szablon"> <i class="fas fa-times"></i> </button>`;
...
newRow.innerHTML = ` <td class="drag-column no-print"><div class="drag-handle"><i class="fas fa-grip-vertical"></i></div></td> <td><input type="text" class="drug-input fluid-name" placeholder="Płyn" list="fluidsList" autocomplete="off" onchange="fillFluidData(this, '${rowId}')" /></td> <td class="additives-cell"> <div class="additive-row"> <select class="additive-select" onchange="updateAdditiveAmount(this, 1)"> <option value="">Wybierz dodatek...</option> </select> <input type="text" class="additive-amount" placeholder="ilość" disabled /> <span class="additive-unit"></span> </div> <div class="additive-row"> <select class="additive-select" onchange="updateAdditiveAmount(this, 2)"> <option value="">Wybierz dodatek...</option> </select> <input type="text" class="additive-amount" placeholder="ilość" disabled /> <span class="additive-unit"></span> </div> </td> <td><input type="number" class="drug-input" placeholder="ml" autocomplete="off" id="${rowId}_vol" oninput="markAsChanged()" onchange="updateSummaries()" /></td> <td><input type="number" class="drug-input fluid-rate" placeholder="ml/h" autocomplete="off" id="${rowId}_rate" oninput="markAsChanged()" onchange="updateSummaries()" /></td> <td><div class="signature-box-cell"></div></td> <td class="action-column no-print"><button onclick="removeRow(this)" class="remove-button"><i class="fas fa-times-circle"></i></button></td>`;

```
tbody.appendChild(newRow); 

// Wypełnij opcje dodatków dla obu selectów
const selects = newRow.querySelectorAll('.additive-select');
selects.forEach(select => {
    Object.keys(additivesData).forEach(additive => {
...
// Nowa funkcja do obsługi dodatków
function updateAdditiveAmount(selectElement, additiveNumber) {
const row = selectElement.closest(’.additive-row’);
const amountInput = row.querySelector(’.additive-amount’);
const unitSpan = row.querySelector(’.additive-unit’);
const selectedAdditive = selectElement.value;

```
if (selectedAdditive && additivesData[selectedAdditive]) {
    const data = additivesData[selectedAdditive];
    amountInput.disabled = false;
    amountInput.placeholder = data.commonDoses[0]; // Domyślna sugerowana dawka
    unitSpan.textContent = data.unit;
    
    // Dodaj datalist z sugerowanymi dawkami
    let datalistId = `additive-doses-${additiveNumber}-${Date.now()}`;
    ...
    amountInput.setAttribute('list', datalistId);
    
    // Automatycznie wstaw pierwszą sugerowaną dawkę
    if (!amountInput.value) {
        amountInput.value = data.commonDoses[0];
    }
} else {
    amountInput.disabled = true;
    amountInput.value = '';
    amountInput.placeholder = 'ilość';
    unitSpan.textContent = '';
    amountInput.removeAttribute('list');
}
...
// Funkcja do generowania tekstu dodatków (dla kompatybilności wstecznej)
function getAdditiveText(row) {
const additiveRows = row.querySelectorAll(’.additive-row’);
const additives = [];

```
additiveRows.forEach(additiveRow => {
    const select = additiveRow.querySelector('.additive-select');
    const amount = additiveRow.querySelector('.additive-amount');
    const unit = additiveRow.querySelector('.additive-unit');
    
    if (select.value && amount.value) {
        additives.push(`+ ${select.value} ${amount.value}${unit.textContent}`);
    }
});
...
// Funkcja do ustawiania dodatków z tekstu (dla kompatybilności wstecznej)
function setAdditivesFromText(row, additiveText) {
if (!additiveText) return;

```
const additiveRows = row.querySelectorAll('.additive-row');
const additives = additiveText.split(' | ').map(add => add.trim());

additives.forEach((additive, index) => {
    if (index < additiveRows.length && additive.startsWith('+')) {
        // Usuwamy znak + i parsujemy resztę
        const cleanAdditive = additive.substring(1).trim();
        
        // Szukamy wzorca: "Nazwa ilość+jednostka"
        // np. "KCl 15% 10ml" -> name="KCl 15%", amount="10", unit="ml"
        const match = cleanAdditive.match(/^(.+?)\s+([\d,\.]+)(\w*)$/);
        
        if (match) {
            const [, name, amount, unit] = match;
            const select = additiveRows[index].querySelector('.additive-select');
            const amountInput = additiveRows[index].querySelector('.additive-amount');
            
            // Znajdź dokładnie pasujący dodatek w liście
            let foundOption = null;
            for (const option of select.options) {
                if (option.value === name.trim()) {
                    foundOption = option;
                    break;
                }
            }
            ...
            if (foundOption) {
                ...
                updateAdditiveAmount(select, index + 1);
...
const newRow = document.createElement(‘tr’);
newRow.innerHTML = ` <td class="drag-column no-print"><div class="drag-handle"><i class="fas fa-grip-vertical"></i></div></td> <td><input type="text" class="drug-input" placeholder="Godz." list="timesList" autocomplete="off" oninput="markAsChanged()" /></td> <td><input type="text" class="drug-input" placeholder="Nazwa procedury/zabiegu" list="proceduresList" autocomplete="off" oninput="markAsChanged()" /></td> <td><div class="signature-box-cell"></div></td> <td class="action-column no-print"><button onclick="removeRow(this)" class="remove-button"><i class="fas fa-times-circle"></i></button></td>`;
tbody.appendChild(newRow);
markAsChanged();
initializeSortable();
}
...
(tables.periodic || []).forEach(data => {
    ...
    if (nameInput) fillPeriodicDrugData(nameInput);
});

// ZAKTUALIZOWANA sekcja fluids z nowym systemem dodatków:
(tables.fluids || []).forEach(data => {
    ...
    if (fluidNameInput) fluidNameInput.value = data.name;
    if (volumeInput) volumeInput.value = data.volume || '';
    if (rateInput) rateInput.value = data.rate || '';
    
    // Ustaw dodatki z nowym systemem
    if (data.additives) {
        setAdditivesFromText(newRow, data.additives);
    }
});
...
document.querySelectorAll('#fluidsTable tbody tr').forEach(row => {
    const fluidNameInput = row.querySelector('.fluid-name');
    const volumeInput = row.querySelector('input[id$="_vol"]');
    const rateInput = row.querySelector('input[id$="_rate"]');
    const additiveText = getAdditiveText(row);
    
    cardState.tables.fluids.push({
        name: fluidNameInput?.value || '',
        additives: additiveText,
...
document.addEventListener('change', function(e) {
    ...
    // Główne przeliczenia i walidacje są teraz tutaj (zdarzenie 'change').
