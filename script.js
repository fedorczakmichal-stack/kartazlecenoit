...
function fillPeriodicDrugData(input) { 
    ...
    if (originalData) { 
        ...
        routeInput.placeholder = ''; // Usuń placeholder gdy jest wartość
        freqInput.value = originalData.frequency; 
        freqInput.placeholder = ''; // Usuń placeholder gdy jest wartość
    } else { 
        doseInput.dataset.originalDose = ''; 
        routeInput.placeholder = 'i.v.'; // Przywróć placeholder gdy brak wartości
        freqInput.placeholder = 'q24h'; // Przywróć placeholder gdy brak wartości
    } 
    recalculateDose(row); 
...
document.addEventListener('DOMContentLoaded', () => { 
    ...
    // Synchronizacja pola sala/łóżko z wersją do wydruku
...
function populateCardFromState(cardState) {
    ...
    // Synchronizuj pole sala/łóżko z wersją do wydruku
    const roomInput = document.getElementById('roomInput');
    const roomInputPrint = document.getElementById('roomInputPrint');
...
function clearCard(force = false) { 
    if (force || confirm('Czy na pewno chcesz wyczyścić całą kartę?')) { 
        ...
        // Wyczyść też pole sala/łóżko do wydruku
        const roomInputPrint = document.getElementById('roomInputPrint');
