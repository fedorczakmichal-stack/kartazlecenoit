...
const gfrDoseAdjustments = {
    ...
    'LEWOFLOKSACYNA': [
        { gfrMax: 50, dose: '500mg x1, potem 250mg', frequency: 'q24h' }
    ],
    'FLUKONAZOL': [
...
function fillPeriodicDrugData(input, rowId) { const drugName = input.value.toUpperCase(); const row = input.closest('tr'); if (periodicDrugsData[drugName]) { const data = periodicDrugsData[drugName]; const doseInput = document.getElementById(rowId + '_dose'); document.getElementById(rowId + '_route').value = data.route; document.getElementById(rowId + '_freq').value = data.frequency; const weight = parseFloat(document.getElementById('patientWeight').value); if (data.dose.includes('/kg') && weight > 0) { const doseRegex = /([\d\.]+)(?:\s*-\s*([\d\.]+))?/; const matches = data.dose.match(doseRegex); if (matches) { const dose1 = parseFloat(matches[1]); const totalDose1 = Math.round(dose1 * weight); if (matches[2]) { const dose2 = parseFloat(matches[2]); const totalDose2 = Math.round(dose2 * weight); doseInput.value = `${totalDose1}-${totalDose2}mg (${data.dose})`; } else { doseInput.value = `${totalDose1}mg (${data.dose})`; } } } else { doseInput.value = data.dose; } } adjustSingleDoseForGfr(row); }
...
// --- LOGIKA DOSTOSOWANIA DAWEK DO GFR ---
...
function adjustSingleDoseForGfr(row) {
    ...
    if (!gfr || !adjustmentRules) {
        if (originalDrugData) { fillPeriodicDrugData(drugNameInput, drugNameInput.id.replace('_name', '')); }
...
function recalculateAllKgDoses(){ document.querySelectorAll('#periodicDrugsTbody tr').forEach(row => { const drugNameInput = row.querySelector('.drug-name'); if(drugNameInput && drugNameInput.value) { fillPeriodicDrugData(drugNameInput, drugNameInput.id.replace('_name', '')); } }); }
