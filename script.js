...
function saveCard() {
    ...
    let isValid = true;
    if (!patientName) {
        patientNameInput.classList.add('field-error');
        isValid = false;
    } else {
    ...
    if (!historyNumber) {
        historyNumberInput.classList.add('field-error');
        isValid = false;
    } else {
    ...
    if (!isValid) {
        showToast('Brak danych', 'Proszę wypełnić Imię i Nazwisko oraz Numer Historii.', 'warning');
        return;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('🏥 Medical Card System v2.1 - Enhanced Edition loaded successfully!');
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    fab.innerHTML = '🔍';
    fab.title = 'Szybkie wyszukiwanie leków (wkrótce)';
    fab.onclick = () => { showToast('Wyszukiwanie', 'Funkcja szybkiego wyszukiwania jest w przygotowaniu.', 'info'); };
